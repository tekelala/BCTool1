...
with st.container():
    if st.session_state.email == "":
        with st.form(key='user_input_form'):
            ...
            user_speech_act = st.multiselect("The main purpose of the message is to make:", ['an assertion','a promise', 'a request'], key="user_multiselect_speech_act")
            message_user = st.text_input("What is the core message you want to send:", key="message_user")
            message_tone = st.multiselect("What is the tone of the message:", ['formal','informal', 'neutral', 'urgent'], key="message_tone")
            
            submit_button = st.form_submit_button(label='Send_container_2')

            if submit_button and user_name and destinatary and user_speech_act and message_user and message_tone:
...
with st.container():
    if st.session_state.email != "":
        with st.form(key='user_text_form'):
            ...
            submit_button = st.form_submit_button(label='Send_container_4')
