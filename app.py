...
with st.container():
    if st.session_state.email == "":
        st.session_state.user_name_input = st.text_input("What is your name:", key="user_name_input")
        st.session_state.destinatary = st.text_input("Who will receive this email:", key="destinatary")
        st.session_state.user_speech_act = st.selectbox("The main purpose of the message is to make:", ['an assertion','a promise', 'a request'], key="user_multiselect_speech_act")
        st.session_state.message_user = st.text_input("What is the core message you want to send:", key="message_user")
        ...
        if submit_button_container_2 and st.session_state.user_name_input and st.session_state.destinatary and st.session_state.user_speech_act and st.session_state.message_user and st.session_state.message_tone:
            st.session_state.user_name = st.session_state.user_name_input
            st.session_state.prompt = f"You are a communications expert using the speech acts of Fernando FLores and you are going to write an email to {st.session_state.destinatary} with the following speech act: {st.session_state.user_speech_act}, with the following {st.session_state.message_tone} the email should transmit the following message {st.session_state.message_user}.\n\n The person that sends the email is: {st.session_state.user_name} Never mention that you use speech acts or that you use a tool to write the email."
...
with st.container():
    if st.session_state.email != "":
        with st.form(key='user_text_form'):
            st.session_state.user_text = st.text_input("Do you want me to make any change to the message?", key="user_text")
            submit_button = st.form_submit_button(label='Send')
            reset_button = st.form_submit_button(label='Restart')

            if submit_button and st.session_state.user_text.strip():
                st.session_state.prompt += f" Please change the email as follows: {st.session_state.user_text.strip()} \n\n remember that the email is sent by \n {st.session_state.user_name}. Write only the email content. Never mention that you use speech acts or that you use a tool to write the email."
